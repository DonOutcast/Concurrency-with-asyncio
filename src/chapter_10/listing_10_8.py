import asyncio
from typing import List, Dict, Optional, Set, Awaitable
    ...
    async with ClientSession() as session:
        ...
        if products in pending:
            ...
            return web.json_response(
                {
                    "error": "Не удалось подключится к сервису товаров."
                },
                status=504
            )
        elif products in done and products.execption() is not None:
            [request.cancel() for request in requests]
...
async def get_products_with_inventory(session: ClientSession, product_response) -> List[Dict]:
    ...
    for done_task in inventory_done:
        if done_task.exception() is None:
            ...
            logging.exception(
                f"Не удалось получть сведения о наличии товаров {product_id}",
